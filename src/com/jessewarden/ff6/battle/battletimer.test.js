...
describe('#battleTimer', ()=>
{
	...
	describe("#Timer", ()=>
	{
		...
		beforeEach(()=>
		{
			timer = new Timer(window, performance);
		});
		afterEach(()=>
		{
			timer.stopTimer();
		...
		{
			let total = 0;
			timer.startTimer((time, difference, now, previousTick)=>
			{
				// log("**********");
				// log("time:", time);
	...
	describe("#BattleTimer", function()
	{
		...
		beforeEach(()=>
		{
			battleTimer = new BattleTimer(window, performance);
		});
		afterEach(()=>
		{
		...
		{
			battleTimer.startTimer(()=>{}, ()=> done());
		});
		it.skip('goes to 100 percent eventually', (done)=>
		{
			const fastTimer = new BattleTimer(window, performance, 0, 0, EFFECT_NORMAL, MODE_PLAYER, 255);
			fastTimer.startTimer(()=> done());
		});
	});
});